...
Command used to send output to the user. Any context variable in the text to say will be replace by its value (see examples below).
...
<CommandDetails
  ...
  examples={[
    ...
    {
      example: "say Hi {{userName}}",
      text: "will replace <i>{{userName}}</i> by the value of evaluating the expression <i>userName</i> using the variables available in the context, if any, and send the resulting text to the user",
    },
    {
      example: "say Hi {{user.name}}",
      text: "will replace <i>{{user.name}}</i> by the value of evaluating the expression <i>user.name</i> using the variables available in the context.",
    },
  ]}
/>
...
the language of the input before the NLP evaluates it.
...
:::info
[Directline connector](/connectors/directline-connector) and [Microsoft Bot Framework](/connectors/msbf-connector)
...
Restarts the conversation and clean up the dialog stack
...
The condition to be evaluated will take into account any variable available in the context.
...
Use the _call_ command to execute a [_custom action_](/bot-orchestration/custom_actions) at any point of a conversation
...
The _set_ command allows you to define a variable within the context.
Remember that _context_ variables are available all over the flow, and can be referenced in many places
[conditions](#conditional-execution), [_say_](#say) commands, [custom actions](/bot-orchestration/custom_actions),
[custom validators](/bot-orchestration/custom_validators), etc.
...
<CommandDetails
  ...
  examples={[
    {
      example: 'set count 5',
      text: 'set <i>count</i> variable to 5'
    },
    {
      example: "set user {name: 'John', surname: 'Doe'}<br>say {{user.name}}",
      text: "set <i>user</i> variable to have the JSON value {name: 'John', surname: 'Doe'} and says 'John'"
    },
    {
      example: "set numbers []<br>say numbers: {{ numbers.length }}",
      text: "set <i>numbers</i> variable with an empty array and says 'numbers length: 0'"
    },
    {
      example: "set numbers [1,2,3]<br>say numbers: {{ numbers.length }}",
      text: "set <i>numbers</i> variable to have an array with values and says 'numbers length: 3'"
    }
  ]}
 />
...
Given a context  <!-- /key_concepts#context) --> variable, it will be increased by the specified amount.
...
Given a context variable, it will be decreased by the specified amount.
