...
- using the [_ask_](/bot-orchestration/dlg_commands_reference#ask) command
- using custom validators
- using NER

In this section we'll explain how to use the custom validators approach.
...
A _validator function_ will receive the following parameters: session, context, and params.
...
Using the [set](/bot-orchestration/dlg_commands_reference#set) command, we've configured the bot to let validators
...
- we can restart the conversation using the [restart](/bot-orchestration/dlg_commands_reference#restart) command, which will
  reset the conversation as if you just opened the bot
- we can set a [condition](/bot-orchestration/dlg_commands_reference#conditional-execution) before the _"say Fine, adding a
  {{pizza}} pizza to your basket."_ command.
  Following the [condition](/bot-orchestration/dlg_commands_reference#conditional-execution) approach, we'd end up with the
