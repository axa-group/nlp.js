...
- explain and split how to configure other adapters (?)
- link to /persistence/database
- different collections can be mapped to different adapters using the _registerCollection(name, adapterName)_ method
...
| autosaveInterval | integer |       0       | In case _autosave_ is true, the periodicity, in milliseconds, to save the contents.<br/>Setting it to 0 will automatically store any change to the filesystem. |
