...
Let's create a file _pizzaTypeValidator.js_ in a _validators_ folder:
...
module.exports = { pizzaType: pizzaTypealidator };
```
...
context. In our case, will add a variable with name `params[0]`, which will correspond to the name of the variable where we
want to store the pizza name, and the pizza name itself as value.
...
Now that we have our validator function, we need to register it into th [bot](/technical-reference/key_concepts#bot) instance.
Let's modify our index.js file like this:

```js title=index.js {5,7,11-13}
const {
  defaultConf,
  axabotsDockStart,
  server,
...
```yaml title=bot-data/flows/basic-script.dlg {10}
