...
DLG is a declarative language used to define conversation flows when using **@bot-core**. Following you'll find a summary 
...
<CommandDetails 
  parameters={[{desc: 'Text or translation key matching the text to send to the user', opt: false}]}
  ...
  examples={[
    ...
    },
    {
      ...
      text: 'where greeting is a key defined in the <a href="../tutorials/translations_files">languages definitions</a>'
    },
    {
...
<CommandDetails 
  parameters={[{desc:'Name of the variable where user input will be stored', opt: true}]}
...
<CommandDetails 
  parameters={[{desc: 'Space-separated list of allowed intents. Leave blank to allow any intent of the corpus to be matched', opt: true}]}
...
```
:::tip
Unfortunately, in the current version of **@bot-core** multiline conditions are not supported, so, if you need a 
