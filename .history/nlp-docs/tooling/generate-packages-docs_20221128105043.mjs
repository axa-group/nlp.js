...
const libProcessing = async (docsPath, readmePath) => {
  ...
  await copyAndReplaceContent(
    readmePath,
    path.join(newPkgPath, "README.md"),
    replacements
  );
...
async function generatePackagesDocs() {
  ...
  const docsProcessings = pkgsWithDocs.map((readmePath) =>
    libProcessing(docsPath, readmePath)
  );
